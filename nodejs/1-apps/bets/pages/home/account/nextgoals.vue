    ...
    <div>
      ...
      <div class="bet-con">
          ...
          <li class="title-ligth">
            <div class="blinks">
              ...
              <span class="bitems cp">
                <span>
                  <span v-if="!checkAdmin && !checkSubadmin">SAHİP</span>
                  <span v-else>
                    <select
                      v-model="userBox"
        ...
        <ul class="bet-summary-con po-r">
          <div
            class="list-bet"
            v-for="(list, index) in filterBy(betSummary, searchBox)"
            :key="index"
            :class="list.state"
          >
            <li class="blinks" @click="getBets($event, index, list)">
              ...
              <span
                class="bitems-list bid"
                :class="{ betIdClass: betid === list.betid }"
                >{{ list.betid }}</span
              >
              <span
              ...
              >
              <span
                v-if="checkAdmin && !checkSubadmin"
                class="bitems-list bowner"
                >{{ list.user | capitalize }}</span
              >
              <span
                v-if="checkBoss || checkSuperadmin"
                class="bitems-list bowner"
                >{{ list.admin | capitalize }}</span
              >
            ...
            <span class="bitems-list bstate">
              ...
              >
                <input
                  v-if="list.userpay === 'no' && list.adminpay === 'yes'"
                  type="button"
                  class="bitems-button bg-b"
                  value="ÖDE"
                  @click="paymentBet(list.id, '', $event)"
                />
                <input
                ...
                />
                <i
                  v-if="list.state === 'Kazandi' && list.userpay === 'yes'"
              ...
              >
                ...
                />
                <i
                  v-if="list.state === 'Kazandi' && list.adminpay === 'yes'"
                  class="fas fa-thumbs-up cl-bl"
                />
              </div>
              <div
                v-if="checkAdmin && !checkSubadmin && list.state === 'Kazandi'"
                class="payment-con"
              >
                <input
                  v-if="list.adminpay === 'no'"
                  ...
                  class="bitems-button bg-b"
                  value="ÖDE"
                  @click="paymentBet(list.id, list, $event)"
                />
                <i
                  v-if="list.state === 'Kazandi' && list.adminpay === 'yes'"
                  class="fas fa-thumbs-up cl-bl"
              ...
              <i v-if="list.state === 'iptal'" class="fas fa-ban" />
              <i v-if="list.state === 'iade'" class="fas fa-retweet" />
              <i v-if="list.state === 'Aktif'" class="fas fa-balance-scale" />
              <i
                v-if="list.state === 'Aktif' && checkAdmin && !checkSubadmin"
                class="fas fa-trash-alt bet-cancel"
                @click="
                  cancelBet($event, list.id, list.amount, list.user, index)
                "
              />
              <i
                v-if="
            ...
            <div class="bet-details-con">
              <div class="bet-details-inside">
                <header class="title-3">
                  ...
                  <i
                    class="fas fa-window-close dialog-close fa-lg"
                    @click="closeDetails()"
                  />
                </header>
                <div class="bet-con">
                  ...
                  <ul class="bet-details po-r">
                    <li
                      ...
                      :class="{
                        ...
                        iptalrepeat: list.betid === cancelInner,
                    ...
                    >
                      ...
                      <span
                        v-if="
                          (checkAdmin && !checkSubadmin) ||
                          checkSuperadmin ||
                          (checkBoss && list.code !== isBetLoader)
                        "
                        class="bitems-list bstate"
                      >
                        <select
      ...
      <div
        v-if="isWarningOpen"
        class="dialog warning warning-bet"
        :style="{ top: top }"
      >
        <header class="cancelbet-header">
          <span class="cancelbet-no">{{ bId }}</span>
        </header>
      ...
      <div v-if="isScore" class="dialog-score" :style="{ top: top }">
        <header class="title-3">
          <h3>Detaylar</h3>
          <i
            class="fas fa-window-close fa-lg dialog-close"
            @click="isScore = false"
          />
        </header>
        <div
...
export default {
  ...
  methods: {
    ...
    async getBetSummary () {
      this.isLoader = true
      await axios
        .post('/api/home/account/betsummarynextgoals', {
    ...
    async removeBet(index) {
      ...
      if (this.$store.state.auth.role !== 'Admin') {
        if (
          this.$store.state.limitsettings.maxkis !== 0 &&
          this.$store.state.limitsettings.maxkisremain ===
            this.$store.state.limitsettings.maxkis
        ) {
          ...
          await axios
            .put('/api/home/account/cancelbet', {
              id: this.bId,
              amount: this.bAmount,
              user: this.bUser,
            })
    ...
    async getBets(event, index, list) {
      ...
      this.betsum = {}
      await axios
        .get('/api/home/account/betsummary/' + list.id)
        .then((result) => {
    ...
    async betInfo () {
      ...
      Promise.all([credit, active, total, won]).then((result) => {
        let final = {
          ...
          ...result[1].data.activeinfo,
          ...result[2].data.totalinfo,
          ...result[3].data.woninfo,
        }
    ...
    async paymentBet(id, list, event) {
      await axios
        .put('/api/home/account/paymentbet', { id: id, list: list })
        .then((result) => {
          if (result.data.code === 200) {
