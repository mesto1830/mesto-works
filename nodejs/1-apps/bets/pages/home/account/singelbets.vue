          ...
          >
            <li class="blinks" @click="getBets($event, index, list)">
              ...
              <span
                ...
                :class="{ betIdClass: betSum.sumid === list.sumid }"
                >{{ list.sumid }}</span
              >
              <span
                ...
                >{{ list.owner }}</span
              >
              <span v-if="checkAdmin" class="bitems-list bowner">{{
                ...
                <div class="bet-con">
                  <ul>
                    <li class="title-ligth">
                      ...
                      <span
                        v-if="checkAdmin || checkSuperadmin || checkBoss"
                        class="bitems bstate"
                        >DURUM</span
                      >
                    </li>
...
export default {
  ...
  methods: {
    ...
    async getBetSummary () {
      this.isLoader = true
      await axios
        .post('/api/home/account/betsummarysingle', { userbox: this.userBox })
        .then((result) => {
          if (result.data.betsummary) {
            this.betSummary = result.data.betsummary.filter((item) => {
    ...
    async getBets(event, index, list) {
      ...
      await axios
        ...
        .then((result) => {
          ...
          const scoreid = result.data.bets.map((item) => item.scoreid)
          if (list.state === 'Aktif') {
