  ...
  <div v-if="checkRole" class="pages">
    ...
    <div class="account-con">
      ...
      <form class="user-id-section" @submit.prevent="register">
        <div class="reg-rows">
          <input type="text" v-model="setUser.user" maxlength="50"  @focus="removeError()" required oninvalid="this.setCustomValidity('Üye alanı boş bırakılamaz!')" oninput="setCustomValidity('')" class="reg-inp" placeholder="Ad">
        </div>
        <div class="reg-rows">
          <input type="text" v-model="setUser.nick" maxlength="50" @focus="removeError()" required oninvalid="this.setCustomValidity('Nick alanı boş bırakılamaz!')" oninput="setCustomValidity('')" class="reg-inp" placeholder="Kod Ad">
        </div>
        <div class="reg-rows">
          <input type="text" v-model="setUser.pass" maxlength="50" required pattern=".{4,}" oninvalid="this.setCustomValidity('Şifre en az 4 karekter olmalıdır!')" oninput="setCustomValidity('')" class="reg-inp" placeholder="Şifre">
        </div>
        <div v-if="checkAdmin || checkSubadmin" class="reg-rows">
          <input v-model="setUser.credit" type="number" min="50" required oninvalid="this.setCustomValidity('Minumum Kredi Tutarı 50 Tl Olmalıdır!')" oninput="setCustomValidity('')" class="reg-inp" placeholder="Bakiye">
        </div>
        <div v-if="!checkAdmin && !checkSubadmin" class="reg-rows">
