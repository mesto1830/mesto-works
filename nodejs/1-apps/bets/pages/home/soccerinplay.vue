        ...
        <div class="sblc">
         ...
         <details :open="isLeagueOpen" class="live-details" v-for="(list, index) in liveList" :key="index">
          ...
          <div v-for="(sublist, index) in list" :key="index" @click="setTop(),getLiveOdds(sublist)" class="live-list soccer">
            ...
            <div v-if="sublist.Scoreboard.period !== 'Finished'" class="left-list-con">
              <span class="llit">
              ...
              <div class="left-list-items cl-y">
                ...
                <i v-if="checkLockTime(minute)" class="fas fa-lock" />
    ...
    <div class="odd-center">
      ...
      <div class="odd-center-con">
        <div v-if="isLiveBet" class="live-center-bet">
          <div  class="center-bet">
            <div class="center-panel po-r">
              <div class="scores-board po-r cp" @click="isEventBoard = ! isEventBoard">
                ...
                <i v-if="isEvent" class="fas fa-window-close event-close fa-lg" @click="closeEvents()"/>
              </div>
              <div v-if="isEvent" class="inplay-events-con">
                <div v-if="inplayEvents" class="inplay-events">
              ...
              <div class="center-header">
                <span class="match-minutes cl-y"><img src="~/static/img/bars.svg" class="live-timer"/>{{minute}} dk</span>
          ...
          <div v-if="checkLockTime(minute)" class="live-lock">
          ...
          <div v-else class="live-suboptions">
            <header class="title-2">
              ...
              <span v-if="activeItem === 'all'" class="badge bg-g ml20">{{allOddsLength}}</span>
              <span v-else class="badge bg-g ml20">{{partOddsLength}}</span>
            </header>
              ...
              <div class="tab-container">
                <ul class="nav nav-tabs nav-justified">
                  <li class="nav-item soccer">
                    ...
                    <a 
                      class="nav-link nll" 
                      @click.prevent="setActive('fulltime'), getFullTime()" 
                      :class="{ active: isActive('fulltime') }" 
                      href="#fulltime">Maç Bahsi
                    </a>
                    <a 
                      class="nav-link nll" 
                      @click.prevent="setActive('goals'), getGoals()" 
                    ...
                    <a 
                      ...
                      href="#secondhalf">İkinci Yarı
                    </a>
                    <a 
                      class="nav-link nll" 
                      @click.prevent="setActive('home'), getHome()" 
                      :class="{ active: isActive('home') }" 
                      href="#home">Ev
                    </a>
                    <a 
                      class="nav-link nll" 
                      @click.prevent="setActive('away'), getAway()" 
                      :class="{ active: isActive('away') }" 
                      href="#away">Dep
                    </a>
                    <i class="fas fa-adjust navbar-icons ml10 cp cl-w" @click="isMarketOpen = ! isMarketOpen" title="Aç Kapa"/>
                  </li>
                  ...
                  <div>
                    <details :open="isMarketOpen" class="suboptions-scroll po-r" v-for="(oddsname, index) in orderBy(liveoddsSort2,'name.value')" :key="index">
                      ...
                      <div v-if="oddsname.visibility === 'Visible'" class="subodds-list subodds-wrapperbwin">
                        <div class="subodds-wrapper prebwin"  v-for="(oddslist, index) in oddsname.results" :key="index" 
                          :class="{liveClass4:oddsname.templateId === 4720 
                          || oddsname.templateId === 4721 || oddsname.templateId === 26644 
                          || oddsname.templateId === 4733 || oddsname.templateId === 19193 
                          || oddsname.templateId === 4734 ||oddsname.templateId === 4727 
                          || oddsname.templateId === 4732 || oddsname.templateId === 4718}" 
                          @click="addSlip(oddslist.odds,oddslist.name.value,oddsname.name.value,oddsname.templateId)">
                          <span class="oddsrate">{{oddslist.name.value}}</span>
                          <span class="oddsrate cl-r">{{oddslist.odds | convertOdd($store.state.newrate)}}</span>
                        </div>
                ...
                <div id="fulltime" class="tab-pane" :class="{ activeshow: isActive('fulltime') }">
  ...
  methods: {
    ...
    async liveOddsFunc (){
      ...
      await axios.get(`/api/home/inplayodds/${this.liveEvents.Id}`).then((result) => {
        ...
        this.liveoddsSort1 = this.liveOdds.filter(item => item.templateId === 17 
          || item.templateId === 21 
        ...
        this.liveoddsSort2 = this.liveOdds.filter(item => item.templateId !== 17 
          && item.templateId !== 21 
          && item.templateId !== 2488 
          && item.templateId !== 3187 
          && item.templateId !== 24392 
