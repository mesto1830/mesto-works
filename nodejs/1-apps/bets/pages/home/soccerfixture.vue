     ...
     <div class="title-2">
        <div class="filter-con">
          ...
          <div class="filter-icons-mid">
            ...
            <i class="fas fa-globe fa-lg cp cl-g ml10" @click="resetFav()"/>
            ...
            <div class="list odd-list" v-for="(sublist, index) in list" :key="index">
              ...
              <div class="odd-right-list-con">
                <div class="odd-right-list">
                  <div class="ottli rate-text" @click="addSlip($event, sublist.HomeTeam, sublist, 'Maç Bahsi')">{{ sublist.home | convertOdd($store.state.newrate) }}</div>
                  <div class="ottli rate-text" @click="addSlip($event, 'X', sublist, 'Maç Bahsi')">{{sublist.draw | convertOdd($store.state.newrate)}}</div>
                  <div class="ottli rate-text" @click="addSlip($event, sublist.AwayTeam, sublist, 'Maç Bahsi')">{{sublist.away | convertOdd($store.state.newrate)}}</div>
                  <div class="ottli rate-text" @click="addSlip($event, 'Alt 2.5', sublist, 'Toplam Gol Alt/Üst 2.5')">{{sublist.under | convertOdd($store.state.newrate)}}</div>
                  <div class="ottli rate-text" @click="addSlip($event, 'Üst 2.5', sublist, 'Toplam Gol Alt/Üst 2.5')">{{sublist.over | convertOdd($store.state.newrate)}}</div>
                  <div class="ottli rate-text" @click="addSlip($event, 'Var', sublist, 'Karşılıklı Gol')">{{sublist.yes | convertOdd($store.state.newrate)}}</div>
                  <div class="ottli rate-text" @click="addSlip($event, 'Yok', sublist, 'Karşılıklı Gol')">{{sublist.no | convertOdd($store.state.newrate)}}</div>
                </div> 
              </div>
            </div>
          </details>
...
export default {
  ...
  methods: {
      ...
      await axios.get('/api/home/soccerfixture').then((result)=>{
        if (result.data.length > 0) {
          ...
          const favtemp = final.filter(item => item.RegionId === 31 
            ...
            || item.RegionId === 20 
            || item.RegionId === 28)
          this.favorites = _.groupBy(favtemp, 'RegionId')
    ...
    getFav (id) {
      ...
      this.leagues = _.groupBy(test, 'leagueid')
