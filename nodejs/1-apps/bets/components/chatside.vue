      ...
      <ul ref="chatarea" class="chat-area" @dblclick="deleteMsg()">
        <li v-for="list in msgHistory" :key="list.id" class="chat-list">
          <div class="receiver-class" 
            :class="{senderClass: list.sender === $store.state.auth.user, 
            readstateClass: list.readed === 'yes' 
            && list.sender === $store.state.auth.user}">
...
export default {
  ...
  methods: {
    ...
    //   })
    //   this.message  += '~/static/img/' + event.target.files[0].name
    // }
  }
}
</script>
