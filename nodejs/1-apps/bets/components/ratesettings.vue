<template>
  <div class="pages">
    <header v-if="$store.state.auth.role === 'Admin'" class="title-2" id="content">
      <span>{{title}}</span>
    </header>
    <ul>
        ...
        <section class="user-id-section">
          <h3 class="mt20 ml20">Tüm Bayiler İçin Oran Ayarları</h3>
            ...
            <li class="set-li">
              ...
              <i class="fas fa-minus-square  fa-3x cl-r slbs" @click="updateRate('Eksi')" title="Eksilt"/>
              <i class="fas fa-cog  fa-3x cl-f slbs" @click="updateRate('Orjinal')" title="Sıfırla"/>
              <img v-if="loader" src="~/static/img/loader.gif" alt="">
              <i v-if="done" class="fas fa-thumbs-up fa-2x cl-g mb10 ml20"/>
            </li>
