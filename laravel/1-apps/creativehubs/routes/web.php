...
//admin side
Route::middleware('checkSession')->get('/addnode', function() {
    $data['nodes'] =  DB::table('nodes')->get(['id']);
    return view('addnode', $data);
});

Route::middleware('checkSession')->post('/addnode', function(Request $req) {
...
Route::middleware('checkSession')->post('/connectedge', function(Request $req) {
...
Route::middleware('checkSession')->get('/addstyle', function(Request $req) {
    $data['nodes'] =  DB::table('nodes')->get(['id']);
    $data['source'] =  DB::table('nodes')->groupBy('source')->get(['source']);
    return view('addstyle', $data);
});

Route::middleware('checkSession')->post('/getstyle', function(Request $req) {
    $data['nodes'] =  DB::table('nodes')->where('source', $req->source)->distinct()->get(['bg','bc','bw']);
    return  $data;
});

Route::middleware('checkSession')->post('/addstyle', function(Request $req){
    if($req->includeChild){
...
Route::middleware('checkSession')->get('/nodes', function(){
    $data['nodes'] =  DB::table('nodes')->get();
    $data['edges'] =  DB::table('edges')->get();
    return view('nodes', $data);
});

Route::middleware('checkSession')->get('/edges', function(){
    $data['edges'] =  DB::table('edges')->get();
