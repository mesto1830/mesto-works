          ...
          <table class="table admin-table">
              ...
              <tbody>
                  @foreach($result as $item)
                      <tr class="list-rows">
                          <td scope="col" class="col-2" style="text-align: left;"><img src="{{ asset('/uploads/activities/'.$item->title.'/'.$item->img)}}" alt="" width="60"></td>
...
<script>
    const deleteActivity = async (id,activiti, e) => {
        ...
        if (confirm(check) == true) {
            const result = await fetch('/admin/activity/delete',{
              method:'POST',
              headers: {
